...
router.post('/create-todo-list', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(201).json({ todoListId: todoList._id, shareKey, name: todoList.name }); // Modified to return the name of the created todo list
  } catch (error) {
    console.log(error);
...
router.post('/join-todo-list', authMiddleware, async (req, res) => {
  try {
    ...
    if (!todoList.users.includes(userId)) {
      ...
      await todoList.save();
      await User.findByIdAndUpdate(userId, { $push: { todoLists: todoList._id } });
      res.status(200).json({ todoListId: todoList._id }); // Ensuring the correct status code is returned when successful
    } else {
      res.status(400).json({ error: 'User already a member of this TODO list' });
    }
  } catch (error) {
